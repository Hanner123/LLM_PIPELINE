...
def create_test_dataloader(data_path, batch_size, device):
    ...
    test_loader = DataLoader(
        ...
        pin_memory=True,
        drop_last=True,
        num_workers=0  # Set to 0 for CPU, adjust if using GPU
    )
    return test_loader
...
if __name__ == "__main__":
    ...
    batch_sizes = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]  # Beispiel-Batchgrößen

    context=0
    throughput_log, latency_log, latency_log_batch = calculate_latency_and_throughput(context, batch_sizes)

    throughput_results = Path(__file__).resolve().parent.parent / "throughput" / "INT8_tensorrt" /"throughput_results.json"
    throughput_results2 = Path(__file__).resolve().parent.parent / "throughput"/ "INT8_tensorrt" / "throughput_results_2.json"
    latency_results = Path(__file__).resolve().parent.parent / "throughput" / "INT8_tensorrt"/ "latency_results.json"
    latency_results_batch = Path(__file__).resolve().parent.parent / "throughput" / "INT8_tensorrt"/ "latency_results_batch.json"
