...
def build_tensorrt_engine_from_trt_file(trt_model_path):
...
def calculate_latency_and_throughput(context, batch_sizes, onnx_model_path):
    ...
    for batch_size in batch_sizes:
        engine, context = build_tensorrt_engine_from_trt_file(onnx_model_path)
...
if __name__ == "__main__":
    # onnx_model_path = Path(__file__).resolve().parent.parent / "models" / "tinybert_quantized.onnx"
    data_path = Path(__file__).resolve().parent.parent / "datasets" / "tokenized_agnews_test.pt"
    trt_model_path = "quantized_model_int8.trt"
    engine, context = build_tensorrt_engine_from_trt_file(trt_model_path)
    ...
    context=0
    throughput_log, latency_log, latency_log_batch = calculate_latency_and_throughput(context, batch_sizes, trt_model_path)
