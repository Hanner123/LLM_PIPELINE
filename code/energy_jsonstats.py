...
def log_jsonstats(log_path, stop_event):
    """Loggt Metriken über jtop (jsonstats) in eine Datei."""
    with jtop() as jetson:
        with open(log_path, 'w') as f:
            while jetson.ok() and not stop_event.is_set():
                stats = jetson.stats
                timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
                stats['timestamp'] = timestamp
                f.write(f"{stats}\n")
                f.flush()
                time.sleep(1)  # jede Sekunde loggen
...
if __name__ == "__main__":
    ...
    # Start jsonstats logging
    jsonstats_log = Path("jsonstats.log")
    stop_event = threading.Event()
    log_thread = threading.Thread(target=log_jsonstats, args=(jsonstats_log, stop_event))
    log_thread.start()

    time.sleep(2)  # kurz warten, bis jtop initialisiert ist
    ...
    print(f"jsonstats output wurde in {jsonstats_log} geschrieben.")

    # Beispiel: letzte 5 Einträge anzeigen
    with open(jsonstats_log, 'r') as f:
        lines = f.readlines()
        print("Letzte 5 Einträge aus jsonstats:")
        for line in lines[-5:]:
            print(line.strip())
